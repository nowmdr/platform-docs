...
| `--fw-bold` | 700 | только крупные заголовки (напр. hero `h1`) |
...
`'use client'`, обёртка над `next/image`: если файл не загрузился (типично —
протухший внешний URL с Amazon), вместо системной «сломанной картинки» с alt-текстом
рендерится нейтральный плейсхолдер (`--color-surface` + полупрозрачный `LogoMark`).
Используется в `ProductCard` и `PostCard` (fill-картинки).
...
`'use client'`. Инпут в стиле поиска (pill) + кнопка `Button`. Бэкенда пока нет —
на сабмит показывает подтверждение. Живёт в футере.
