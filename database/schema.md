...
⚠️ **Схемный дрейф — миграции, применённые к base-one через MCP из админки, но ещё НЕ
продублированные файлами в репозитории сайта**: `add_admin_folders` (таблица
`admin_folders` + `media.folder_id`), `add_content_folders` (`products.folder_id`,
`posts.folder_id` + FK/индексы). При следующем касании миграций сайта — добавить файлы
(идемпотентно, `if not exists`), чтобы история осталась полной.
...
1. Новый файл миграции в `cozycorner/supabase/migrations/` (таблицы контента — в схеме
   ...
   репозитории сайта (см. дрейф в §6).
