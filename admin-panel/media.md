...
> Last updated: 2026-07-22 | Source project: web.admin (docs/media-manager.md) — пути `src/…` относятся к репозиторию `web.admin/`
...
- **UI** (`src/features/folders/` — общий код разделов):
  ...
  - **Multi-select**: чекбокс всегда виден (на превью карточки в media / в строке
    ...
    элементами, чтобы скрытые чекбоксы не участвовали в перемещении/удалении
    незаметно. Одиночный move убирает из выборки только перемещённые id; смена
    активной папки и удаление элемента сбрасывают/чистят выборку.
  - **Bulk delete** — общая кнопка `BulkDeleteButton` (destructive, в bulk-баре
    между «Move to…» и «Clear») + общий хук `useBulkDelete<T>` (`useBulkDelete.ts`):
    последовательное удаление выбранных (ошибка одного не рвёт остальные, тост об
    успехе + отдельные тосты об ошибках), инвалидация `contentKey` и чистка выборки.
    Доменная `deleteOne` приходит из раздела (`deleteImage`/`deleteProduct`/
    `deletePost`). Для media в confirm-диалог передаётся `warning`, если часть
    выбранных используется на сайте (по `usedBy`) — предупреждает, но не блокирует.
  ...
  - **effectiveFolder** — если активная папка была удалена в другой сессии (второй
    ...
    мёртвом id.
  - `ImagePickerDialog` (пикер картинок для товаров/постов/страниц) **имеет фильтр
    по папкам** (2026-07-22): компактный `Select` (All / Unsorted / папки со
    счётчиками) слева от поиска. Клик фильтрует грид по `folder_id`; поиск, как и в
    разделе Media, идёт сквозь все папки. Upload грузит в выбранную папку
    (`uploadImage(site, file, folderId)`; All/Unsorted → `null`). Активная папка —
    локальный стейт диалога, сбрасывается в All при закрытии. CRUD папок в пикере
    нет (создание/переименование/удаление — только в разделе Media) — сознательно.
