...
«N selected · Move to… · Delete · Clear» (`BulkDeleteButton` + `useBulkDelete`,
confirm-диалог «Delete N products?»). Семантика фильтров: поиск — сквозь все папки,
...
- **Грид-шапка формы** `md:grid-cols-[16rem_1fr]` (на mobile — одна колонка):
  ...
  же пикер (2026-07-22). Под превью read-only slug; справа — Title*, Price*
  (число ≥ 0) + Brand в одной строке, Image path (инпут + outline-кнопка
  **Gallery** → диалог-пикер, см. ниже). `ImagePreviewPicker` сам ведёт `broken`-
  состояние и сбрасывает его при смене `url` — call site стейт не отслеживает.
...
Bulk-бар (Move to… + Delete) и семантика фильтров — как в Products (поиск
сквозь папки, Status — AND с папкой). Состояния: 8 skeleton-строк; «No posts
yet.»; «No posts match
...
- **Bulk-удаление** — общий `BulkDeleteButton` (destructive, в bulk-баре) +
  хук `useBulkDelete<T>` (`src/features/folders/useBulkDelete.ts`): доменная
  `deleteOne` из раздела, последовательное удаление, инвалидация + чистка выборки.
