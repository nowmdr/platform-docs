...
> Last updated: 2026-07-17 | Source project: web.admin (docs/ux-ui.md, актуально на 2026-07-16) — пути `src/…` относятся к репозиторию `web.admin/`
...
- Высота 14, нижняя граница; контент по центру `max-w-5xl`.
...
- Меню (WP-паттерн): **Media / Pages / Products / Blog / SEO Posts**, у пункта иконка
...
Unsorted». Чекбокс выборки — всегда видим в левом верхнем углу превью карточки;
при непустой видимой выборке над гридом появляется **bulk-бар** «N selected ·
Move to… · Clear» (`MoveToFolderMenu`). Одиночное перемещение — кнопка Move в
футере карточки и строка Folder в модалке деталей (текущая папка исключена из
целей). Upload кладёт файлы в открытую сейчас папку (All/Unsorted → без папки).
...
чекбокс выборки (сосед ссылки, не вложен в неё) + строка-ссылка (сортировка
...
`currentFolderId` строки). При непустой видимой выборке над списком — bulk-бар
«N selected · Move to… · Clear». Семантика фильтров: поиск — сквозь все папки,
...
- **Грид-шапка формы** `md:grid-cols-[16rem_1fr]` (на mobile — одна колонка):
  слева квадратное превью картинки (`resolveImageUrl`, object-contain;
  плейсхолдер ImageOff при пустом/битом пути) и под ним read-only slug; справа —
  Title*, Price* (число ≥ 0) + Brand в одной строке, Image path (инпут + живое
  превью + outline-кнопка **Gallery** → диалог-пикер, см. ниже).
- Ниже грида на всю ширину формы (`max-w-3xl`): Referral URL* (валидный URL),
  Category (Select из `categories.name` + None) + Image style (Select
  photo/cutout) в одной строке, Description (markdown-редактор, см. ниже),
  секция SEO. Пустые optional-поля пишутся в БД как `null`. Невалидный Save —
...
- Пикер картинки (`ImagePickerDialog`, `sm:max-w-3xl`): поиск по имени/пути,
  кнопка **Upload** (один файл, `uploadImage` из `lib/media` — валидация,
  sanitize, откат; после загрузки путь сразу подставляется в товар и диалог
  закрывается), грид миниатюр 3/4/5 колонок из `listImages` (общий query-кэш
  `['media', site.slug]` с разделом Media), текущая картинка подсвечена ring.
  Клик по миниатюре → `image_path` в форму (`setValue`, `shouldDirty`);
  запись в БД — только по Save.
...
чекбокс + строка-ссылка (сортировка `published_at desc` + tiebreaker
`title asc`): title слева, справа приглушённо бейдж **Draft** (при
`is_published=false`) + дата `formatDate(published_at)`; справа иконка Move.
Bulk-бар и семантика фильтров — как в Products (поиск сквозь папки, Status —
AND с папкой). Состояния: 8 skeleton-строк; «No posts yet.»; «No posts match
...
- **Sticky-панель**: слева «← Back to blog»; справа тоггл **Published/Draft**
  ...
  только в панели.
- **Грид-шапка**: слева превью cover (`aspect-video`, на md+ квадрат) + slug;
  справа Title*, Excerpt (textarea), Cover image (инпут публичной ссылки +
  **Gallery** → общий `ImagePickerDialog` из products).
...
edit; sticky-панель Back + Save; грид-шапка (превью OG image + slug слева; SEO
title / SEO description / OG image + Gallery справа); Body (`RichTextEditor`,
только terms/privacy); hero-секции (`HeroSectionsEditor` — только update
существующих, глазик/badge/title/description/bg image/align left|right).
...
- Контент страницы — `max-w-5xl`, паддинг 4; шапка h-14.
...
## 6. Будущие разделы (Categories, footer_settings, …)
