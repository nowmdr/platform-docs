...
- **Грид-шапка формы** `md:grid-cols-[16rem_1fr]` (на mobile — одна колонка):
  ...
  (число ≥ 0) + Brand (`TaxonomyCombobox`) в одной строке, Image path (инпут + outline-кнопка
...
- Ниже грида на всю ширину формы (`max-w-3xl`): Referral URL* (валидный URL),
  Category (`TaxonomyCombobox` — выбор/создание + Manage) + Image style (Select
...
Manage… открывает `TaxonomyManager` в модалке).
